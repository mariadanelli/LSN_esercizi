...
for (int j=0; j<N; j++) {
  ...
  do {
   x2=rnd.Rannyu(-1,1);       //genero casualmente l'angolo con cui cade l'ago
   y2=rnd.Rannyu(-1,1);       // utilizzando una x e una y all'interno della circonferenza unitaria, in modo da poter usare il seno e il coseno
   ...
   if (x1+A*cos(theta)<0 || x1+A*cos(theta)>d)
   Nhit++;
   Ntot++;

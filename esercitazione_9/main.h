...
struct Individuo {
    ...
    double Costo(Posizione citta[N_cities]) {
       double L=0;
    for (int k=1; k<N_cities-1; k++) {
    int i= Ind[k];
    int j= Ind[k-1];
    L= L+citta[i].Distanza(citta[j]);
    }
    return L+citta[N_cities-1].Distanza(citta[0]);
    } ;
};
...
void Mutation2(Individuo& I) {
  double p= rnd.Rannyu(0,10);
   if (p<0.91) ScambioCasuale(I);
   if (p>=0.91 && p<1.82) Shift(I);
   if (p>=1.82 && p<2.73) Invers(I);
   if (p>=2.73 && p<3.64) Permut(I);
}

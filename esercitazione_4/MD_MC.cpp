...
// distance i-j in pbc
  ...
  walker[it] = (2.0 / 3.0) * kin/(double)npart; // Temperature //uso meccanica statistica
  walker[ie] = 4.0 * v + kin;  // Total energy;
  walker[ip] = rho*temp+ 48/(3*vol)*p; //Pressure
  return;
}
...
void Averages(int iblk) //Print results for current block
{
   ...
   ofstream Epot, Ekin, Etot, Press, Temp;
   const int wd=12;
    ...
    if (restart==0) {
    Temp.open("output_tempeq.dat",ios::app); }
    else {Temp.open("output_temp.dat",ios::app); 

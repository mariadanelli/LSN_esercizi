...
// distance i-j in pbc
  ...
  walker[iv] = 4.0 * v +npart*((8*pi*rho)/ (9*pow(rcut, 9))-(8*pi*rho)/(3*pow(rcut, 3))) ; // Potential energy con tail correction
  walker[ik] = kin; // Kinetic energy
  walker[it] = (2.0 / 3.0) * kin/(double)npart; // Temperature
  walker[ie] = 4.0 * v + kin;  // Total energy con tail correction;
  walker[ip] = rho*temp+ 48/(3*vol)*p+32*pi*rho*(1./(9*pow(rcut,9))- 1./(6*pow(rcut,3))); //Pressure  con tail correction //boh ci va il 3???
  return;
}

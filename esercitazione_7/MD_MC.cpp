...
void Input(void)
{
 ...
 if (iNVET==0) cout << "MD(NVE) simulation       " << endl << endl;
 else cout << "MC(NVT) simulation    "<< endl<<endl;
...
// distance i-j in pbc
	...
	/*divido per numero di particelle??*/
  ...
  walker[ip] = rho*temp+ 48/(3*vol)*p; //Pressure  //boh ci va il 3???
  return;
}
...
void Averages(int iblk) //Print results for current block
{
   ...
   ofstream Epot, Ekin, Etot, Press, Temp, G, Gf;
   const int wd=12;
	...
	Gf.open("output_gf.dat", ios::app);
    stima_pot = blk_av[iv]/double(npart)/double(blk_norm)+ (8*pi*rho)/ (9*pow(rcut, 9))-(8*pi*rho)/(3*pow(rcut, 3));//Potential energy con tail correction
	...
	stima_press = blk_av[ip]/blk_norm+32.0* pi*rho*(1./(9*pow(rcut,9))- 1./(6*pow(rcut,3))); //Pressure
...
//Temperature
	...
	//uso lo stesso array che per le altre grandezze per calcolare le medie però poi siccome la mia posizione parte da zer devo ogni volta sottrarre il numero di grandezze per far si che l'indice parta da zero
	for (int k=n_props; k<n_props+nbin; k++) {

		double r=deltabin*(k-n_props);
		norm= rho*npart*(4./3.)*pi*(pow(r,3)- pow(r-deltabin,3));
			
	    g= blk_av[k]/norm/blk_norm;
		glob_av[k]+= g;
		glob_av2[k]+= g*g;
		err_g=Error(glob_av[k], glob_av2[k], iblk);
		G<<g<<"    ";
  	if (iblk==nblk) {
		Gf<<r<<setw(wd) << glob_av[k]/(double)iblk<<setw(wd) << err_g<<endl;
		}
	}
  G<<endl;

...
//cycle over spins
  ...
  walker[iu] = u;
	walker[im]= m;
	walker[ix]= m*m; 
	walker[ic]=u*u;// INCLUDE YOUR CODE HERE
}
...
void Averages(int iblk) //Print results for current block
{
  ...
  err_m= Error(glob_av[im], glob_av2[im], iblk);
	Mag<<setw(wd)<<iblk<<setw(wd)<<stima_m<<setw(wd)<<setw(wd)<<glob_av[im]/(double)iblk<<setw(wd)<<err_m<<endl;
  ...
  err_x= Error(glob_av[ix], glob_av2[ix], iblk);
	Chi<<setw(wd)<<iblk<<setw(wd)<<stima_x<<setw(wd)<<setw(wd)<<glob_av[ix]/(double)iblk<<setw(wd)<<err_x<<endl;
